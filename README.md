...
| dance               | bool       |       1        |
| time_video          | uuid       |       16       |
| searched_to_end     | bool       |       1        |
| likes               | uuid       |       16       |
| dislikes            | uuid       |       16       |
| commented           | bool       |       1        |
| views_number        | uuid       |       16       |
| **Итого за запись** |            |     **342**    |
...
- **Таблица `videos` (обработка)**: Для каждого пользователя предполагается наличие 10 записей.
- **Таблица `actions`**: Для каждого пользователя хранится 100 записей.

### Сводная таблица расчетных размеров для хранения данных

| Таблица              | Расчетный размер для хранения (байты) | Расчетный размер для хранения (ГБ) | Расчетный размер для хранения (ТБ) | Запись | Чтение  |
|----------------------|---------------------------------------|------------------------------------|------------------------------------|--------|---------|
| `users`              | 88,000,000,000                        | ~81.9                              | ~0.08                              | 10k    | 200k    |
| `videos`             | 3,344,000,000,000                     | ~3,110.7                           | ~3.11                              | 393    | 2.295k  |
| `videos_feed`        | 3,520,000,000,000                     | ~3,278.4                           | ~3.28                              | 6.9k   | 20.7k   |
| `likes`              | 352,000,000,000                       | ~327.7                             | ~0.33                              | 0.229k | 2.295k  |
| `comments`           | 3,344,000,000,000                     | ~3,110.7                           | ~3.11                              | 0.229k | 1.147k  |
| `subscribers`        | 3,520,000,000,000                     | ~3,278.4                           | ~3.28                              | 0.229k | 0.573k  |
| `videos` (обработка) | 78,341,000,000                        | ~72.9                              | ~0.07                              | 393    | 2.295k  |
| `actions`            | 37,620,000,000,000                    | ~35,037.9                          | ~35.055                            | 10k    | 20.833k |
| `авторизации`        | 74,800,000,000                        | ~69.615                            | ~0.068                             | 0.424k | 6.944k  |
